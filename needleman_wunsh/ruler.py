...
def element_min(L) :
    """Méthode renvoyant l'élément minimum d'une liste et son indice"""
    min = L[0]
    indice = 0
...
class Ruler :
    ...
    #Note : Je n'ai pas suivi le procédé présenté sur wikipedia mais plutôt celui présenté dans une vidéo Youtube, je conseille
    #vivement de la consulter rapidement pour comprendre la construction des matrices de score et de parcours
    ...
    def compute(self) :
        ...
        #Création de la matrice de parcours, elle explicite à partir de quel coefficient un coefficient a été calculé
        #(coefficient supérieur, de gauche ou diagonale supérieure gauche)
        P = np.array([["" for y in range (j+1)] for x in range(i+1)])
        P[0,:] = ["Bord" for k in range(j+1)]
        P[:,0] = ["Bord" for k in range (i+1)]
        #On remplira cette matrice avec : "Diagonale" (ou "D"), "Haut" (ou "H") ou "Gauche" (ou "G") pour indiquer
        #la provenance du coefficient
        ...
        for y in range(1,j+1) :
            for x in range (1,i+1) :
                #Liste des 3 résultats possibles pour remplir la prochaine case de la matrice de score
                #(2 provenants des cases adjacentes et 1 de la case en diagonale)
                Res = []


                Res.append(M[x-1][y-1] + match(self.a[x-1],self.b[y-1]))  #Score si on calcule le coefficient à partir de celui
                                                                         #dans la diagonale supérieure gauche
        ...
        self.distance = M[i][j]         #On lit la distance sur le coefficient dans le coin inférieur droite de la matrice
                                        #(dernier coefficient construit)
    ...
    def report(self) :
        ...
        while x > 0 or y > 0 :
            ...
            # Une séquence a donc été complètée, on complète l'autre
            if P[x][y] == "B" :
                while x > 0 :
                    top = self.a[x-1] + top
            ...
            if P[x][y] == "D" :
                if self.a[x-1] != self.b[y-1] :  #Cas où les 2 nucléotides ne correspondent pas
